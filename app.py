...
def main():
    ...
    # ---------- НОВЫЙ ГРАФИК: Динамика выигранного объёма по датам ----------
    st.subheader("📈 Динамика выигранного объёма по датам")
    if "Объем выигранный" in filtered.columns:
        # Оставляем только строки с выигранным объёмом > 0
        won_over_time = filtered[filtered["Объем выигранный"].notna() & (filtered["Объем выигранный"] > 0)]
        if not won_over_time.empty:
            # Группируем по дате торгов
            won_by_date = won_over_time.groupby("Дата торгов")["Объем выигранный"].sum().reset_index()
            # Преобразуем дату в datetime для правильного порядка
            won_by_date["Дата"] = pd.to_datetime(won_by_date["Дата торгов"], format="%d.%m.%Y")
            won_by_date = won_by_date.sort_values("Дата")
            
            # Строим столбчатый график
            fig_daily = px.bar(won_by_date, 
                               x="Дата", 
                               y="Объем выигранный",
                               title="Выигранный объём по дням",
                               labels={"Объем выигранный": "Выигранный объём (кг)"},
                               text_auto=True)  # показывает значения на столбцах
            # Настраиваем формат оси X
            fig_daily.update_xaxes(tickformat="%d.%m.%Y", title_text="Дата торгов")
